...
class _AbsenScreenState extends State<AbsenScreen> {
  ...
  // Anti-cheat
  int _frameMataTerbuka = 0;
  double _lastEulerY = 0;
  int _frameWajahStabil = 0;
  Rect? _wajahRect;
  Size? _previewSize;
  bool _isCaptured = false; // Flag sudah capture di detik 0.5

  static const int _minFrameMataTerbuka = 5;
  static const double _maxEulerY = 25.0;
  static const double _maxEulerX = 20.0;
  ...
  Future<void> _initKamera() async {
    ...
    try {
      ...
      // Auto exposure biar tidak gelap
      if (_previewSize != null) {
        await _cameraController!.setExposureMode(ExposureMode.auto);
        await _cameraController!.setExposurePoint(
          Offset(_previewSize!.width / 2, _previewSize!.height / 2),
        );
      }
  ...
  Future<void> _prosesFrame(CameraImage image) async {
    if (_memproses || _isCaptured || !mounted) return;

    final now = DateTime.now().millisecondsSinceEpoch;
    if (now - _lastProcess < 200) return;
    _lastProcess = now;
    ...
    try {
      ...
      if (faces.isEmpty) {
        setState(() {
          ...
          _frameMataTerbuka = 0;
          _frameWajahStabil = 0;
          if (_countdown < 2 && _livenessOk) {
            _batalkanCountdown('Wajah hilang!');
          } else if (!_livenessOk) {
      ...
      if (eulerY.abs() > _maxEulerY || eulerX.abs() > _maxEulerX) {
        setState(() {
          ...
          _frameMataTerbuka = 0;
          if (_countdown < 2 && _livenessOk) {
            _batalkanCountdown('Kepala terlalu miring!');
          }
        });
        _memproses = false;
        return;
      ...
      // Anti-cheat: mata harus terbuka cukup lama
      if (rataEye > 0.75) {
        _frameMataTerbuka++;
      } else {
        _frameMataTerbuka = 0;
      ...
      final mataSudahTerbuka = _frameMataTerbuka >= _minFrameMataTerbuka;

      // Anti-cheat: wajah harus stabil saat countdown
      if (_countdown < 2 && _livenessOk) {
  ...
  void _mulaiCountdown() {
    ...
    _countdownTimer = Timer.periodic(const Duration(milliseconds: 500), (t) {
      if (!mounted) {
        t.cancel();
        return;
      }

      // Cek validitas wajah setiap 500ms
      if (!_wajahValid || _wajahRect == null) {
        t.cancel();
        _batalkanCountdown('Wajah tidak terdeteksi!');
        return;
      }
      ...
      setState(() {
        ...
        } else if (_countdown == 1 && !_isCaptured) {
          // CAPTURE DI DETIK 1 (0.5 detik sebelum selesai)
          _instruksi = 'Ambil foto...';
          _isCaptured = true;
          t.cancel();
          _captureDanValidasi();
        }
  ...
  Future<void> _captureDanValidasi() async {
    if (_cameraController == null || !_cameraReady || !_wajahValid) {
      _batalkanCountdown('Wajah tidak valid!');
      return;
    }
    ...
    try {
      ...
      // Kirim langsung ke server (countdown udah 0)
      _prosesTimer = Timer(const Duration(milliseconds: 300), _kirimAbsen);
    } catch (e) {
      setState(() => _pesan = 'Gagal ambil foto: $e');
      _batalkanCountdown('Gagal ambil foto!');
    }
  }
  ...
  void _batalkanCountdown(String alasan) {
    ...
    // Restart stream kalau belum diambil foto
    if (_foto == null) {
      _cameraController?.startImageStream(_prosesFrame);
    }
  }
