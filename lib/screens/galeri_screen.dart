...
class _GaleriScreenState extends State<GaleriScreen> {
  ...
  if (forceRefresh) {
    // Force: langsung fetch server
    await _fetchServer(tanggal, clear: true);
    return;
  }

  // Cek cache dulu
  final hasCache = await _GaleriCache.hasCache(tanggal);
  ...
  if (hasCache) {
    ...
    // Tidak ada cache, fetch server
    await _fetchServer(tanggal, clear: false);
  }
}
...
Future<void> _fetchServer(String tanggal,
  ...
  try {
    ...
    if (mounted) setState(() { _data = []; _loading = false; });
  }
}
